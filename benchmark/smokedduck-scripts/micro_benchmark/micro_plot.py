...
# 2) separate accumelating lineage vs copying overhead for Function-Level
...
if plot_filter:
    ...
    wbaseline = con.execute(f"""select {aug_baseline}
                      ...
                      """).df()
    print(wbaseline)
    mdata = con.execute(f"""select {g}, lineage_type, {m}, {roverheads} from wbaseline""").df()
    ...
    data = con.execute("""select 'SD' as sys_label, * from data where system='Function-Level' UNION ALL 
    SELECT 'Logical' as sys_label, * from data where system='Logical-RID' """).df()
...
if plot_ineq:
    ...
    system_d = { "Function-Level": "SD", "Logical-RID": "Logical"}
...
if plot_agg:
    ...
    mdata = con.execute(f"""select {g}, lineage_type, {m}, {roverheads} from wbaseline""").df()
    print(mdata)
    where = f"where system IN ('Baseline', 'Function-Level', 'Logical-RID', 'Logical-RID_list', 'Smoke')  {cond}"
    data  = con.execute(template.format(g, g, g, where)).fetchdf()
    ...
    """).df()
    sample_data = con.execute("select * from data where overheadType<>'Execute'").df()
    print(con.execute("select * from data where system='Logical-RID'").df())

    sample_data_10m = con.execute("select * from sample_data where card=10000000 and op_name<>'HASH_GROUP_BY' and system<>'Logical-RID_list' and system<>'Smoke'").df()
    ...
    # 1. x-axis: selectivity, y-axis: runtime, facet: cardinality
    system_d = { "Function-Level": "SD", "Logical-RID": "Logical", "Smoke": "Smoke"}
...
plot_data = con.execute("select * from global_df where system in ('Logical-RID', 'Function-Level') and overheadType='Total'").df()
