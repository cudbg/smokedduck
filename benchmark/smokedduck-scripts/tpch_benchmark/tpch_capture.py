...
from utils import parse_plan_timings, Run, getStats, get_lineage_type
...
parser.add_argument('--folder', type=str, help='queries folder', default='benchmark/smokedduck-scripts/queries/')
args = parser.parse_args()
args.profile = True
args.stats = True
print(args.profile)

prefix = args.folder + "/q"
table_name=None
if args.perm:
    prefix = args.folder + "/perm/q" # "Logical-RID"
    if args.opt:
        prefix = args.folder + "/optimized_perm/q" # "Logical-OPT"
elif args.gprom:
    prefix = args.folder + "/gprom/q" # "Logical-window"
elif args.logical_list:
    prefix = args.folder + "/perm_list/q" # "Logical-list"
if args.normalized:
    prefix = args.folder + "/normalized/q" # += "_normalized"
