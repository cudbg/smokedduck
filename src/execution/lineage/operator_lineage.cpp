...
void OperatorLineage::LQ_single(unordered_map<idx_t, vector<idx_t>>& log_context, vector<vector<idx_t>>& iids) {
  for (auto& e : log_context) {
    idx_t pk = e.first;
    idx_t thread_idx = pk % (thread_vec.size() + 1);
    idx_t lsn = pk / (thread_vec.size() + 1);
    vector<idx_t>& oids = e.second;
    bool debug = false;
    if (debug)
      std::cout << "LQ_single tid(" << thread_idx << "), lsn(" << lsn << ")," << " |oids|: " << oids.size() << std::endl;
    ...
    case PhysicalOperatorType::FILTER: {
      ...
      for (auto& oid : oids) {
        if (branch == 0) {
          iids.emplace_back(tlog->filter_log[blsn].sel[oid]);
        } else {
          iids.emplace_back(oid);
        }
      }*/
      break;
      ...
      for (idx_t o=0; o < oids.size(); ++o) {
        idx_t oid = oids[o];
        oids[o] = tlog->reorder_log[lsn][oid];
      }
            ...
            if (type == PhysicalOperatorType::HASH_GROUP_BY) {
              for (int k=0; k < tlog->scatter_log.size(); ++k) {
                ...
                if (!temp_iids.empty()) {
                  ...
                  GetNextChild(children[0])->LQ_single( oids_per_lsn, iids );
                }
              }
            } else {
    ...
    /*  D_ASSERT(lsn < tlog->execute_internal.size());
      ...
      if (!tlog->perfect_probe_ht_log.empty()) {
        ...
        for (auto& oid : oids) {
          if (left == nullptr) { iids.push_back( oid ); } // + in_start to get index into virtual relation
          else {  iids.push_back(left[oid]); }  // + in_start to get index into virtual relation
